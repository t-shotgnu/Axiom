...
describe('ProjectsComponent', () => {
  ...
  it('does not create a project when required fields are blank', () => {
    const { component, loadProjects$ } = createComponent();
    loadProjects$.next([]);
    loadProjects$.complete();
    component.form = { name: ' ', code: ' ', description: '' };
  ...
  it('creates a project with trimmed and normalized form data', () => {
    const { component, loadProjects$ } = createComponent();
    loadProjects$.next([]);
    loadProjects$.complete();
    projectService.createProject.mockReturnValue(of('project-1'));
    ...
    expect(component.createSuccessMessage).toBe('Axiom was created.');
    expect(projectService.getAllProjects).toHaveBeenCalledTimes(2);
  });

  it('omits empty descriptions when creating a project', () => {
    const { component, loadProjects$ } = createComponent();
    loadProjects$.next([]);
    loadProjects$.complete();
    projectService.createProject.mockReturnValue(of('project-1'));
  ...
  it('maps conflict errors while creating projects', () => {
    const { component, loadProjects$ } = createComponent();
    loadProjects$.next([]);
    loadProjects$.complete();
    ...
    expect(component.createErrorMessage).toBe(
      'A project with this code may already exist. Try another code.',
    );
  });
});
