...
public class WorkItemController {
    ...
    public ResponseEntity<Void> assignWorkItem(@PathVariable UUID id, @Valid @RequestBody AssignWorkItemCommand command) {
        commandService.assignWorkItem(id, command.assigneeId());
        return ResponseEntity.ok().build();
    }

    @PatchMapping("/{id}/status")
    public ResponseEntity<Void> updateStatus(@PathVariable UUID id, @Valid @RequestBody UpdateWorkItemStatusCommand command) {
        commandService.updateStatus(id, command.status());
        return ResponseEntity.ok().build();
    }

    @PutMapping("/{id}")
    public ResponseEntity<Void> updateWorkItem(@PathVariable UUID id, @Valid @RequestBody UpdateWorkItemCommand command) {
        commandService.updateWorkItem(id, command);
        return ResponseEntity.ok().build();
    }

    @PatchMapping("/{id}/notes")
    public ResponseEntity<Void> updateNotes(@PathVariable UUID id, @Valid @RequestBody UpdateWorkItemNotesCommand command) {
    ...
    public ResponseEntity<Void> deleteWorkItem(@PathVariable UUID id) {
        commandService.deleteWorkItem(id);
